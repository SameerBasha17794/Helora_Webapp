...
class GMEX_Easy_Payment_Public {
    ...
    public function easy_payment_button_generator($atts, $content = null) {
        ...
        // Custom - in custom field show doctor email (will be used to send an email to the doctor)
        global $bp;
        if( ! bp_is_user() )
        return ;
        $doctorEmail = bp_get_displayed_user_email();
        ...
        // Doctor email from which we are buying the procedure
        //$output .= '<input type="hidden" name="custom" value="' .$doctorEmail. '">';
        $output .= '<input type="hidden" name="custom" value="' .$currentLoggedUserID. '">';
        // Custom END
